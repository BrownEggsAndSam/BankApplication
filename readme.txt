...
Set<String> getCloudEdlDatasetRegistryKeys() {
    ...
    String tvc = """{
        "TableViewConfig": {
            ...
            "Asset": {
                "Domain": {
                "Id": { "id": "df3e2377-468c-43a6-adaf-5cb6b3eeabfd" }
                },
                "StringAttribute": [
                { "labelId": "03feeee22-8a3f-4158-acb2-9660db25c4f2", "LongExpression": { "name": "CMDBAssetId" } },
                { "labelId": "6d5614df-4cd2-4098-b04c-9da52f6401c0", "LongExpression": { "name": "EndpointUrl" } },
                { "labelId": "2de8f8db-4cad-4353-8031-9bc1ba4001e1", "LongExpression": { "name": "DbName" } },
                { "labelId": "0191ddba-3369-7235-bb85-efa85427f99d", "LongExpression": { "name": "SchemaName" } },
                { "labelId": "c1bd6cdd-2a6e-444a-a774-ea754df96e63", "LongExpression": { "name": "TableName" } }
                ]
            }
            },
            "Columns": [
            { "Column": { "fieldName": "CMDBAssetId" } },
            { "Column": { "fieldName": "EndpointUrl" } },
            { "Column": { "fieldName": "DbName" } },
    ...
    for (int i = 0; i < res.size(); i++) {
        String key = "${res[i].CMDBAssetId ?: ''}|${res[i].EndpointUrl ?: ''}|${res[i].DbName ?: ''}|${res[i].SchemaName ?: ''}|${res[i].TableName ?: ''}".toLowerCase().trim()
        registryKeys.add(key)
    }

    if (verboseLogs) loggerApi.info("[PDD Template Export] Found ${registryKeys.size()} existing datasets in Cloud EDL registry.")
    return registryKeys
}
...
try {
    ...
    Set<String> existingRegistryKeys = getCloudEdlDatasetRegistryKeys()
    ...
    new XSSFWorkbook(fInput).withCloseable { workbook ->
        ...
        for (selectedTable in selectedTables) {
            ...
            String schemaName = execution.getVariable("dbSchema")
           
            String rowKey = "${cloudAssetId}|${execution.getVariable("dbUrl")}|${execution.getVariable("dbName")}|${schemaName}|${selectedTable}".toLowerCase().trim()

            if (existingRegistryKeys.contains(rowKey)) {
                for (int i = 0; i < mdHeaderMap.size(); i++) {
                    Cell cell = mdRow.getCell(i, Row.MissingCellPolicy.CREATE_NULL_AS_BLANK)
                    cell.setCellStyle(infoNeededCellStyle)
                    if (verboseLogs) loggerApi.info("[PDD Template Export] Existing dataset found in Cloud EDL. Marking row with infoNeededCellStyle: ${rowKey}")

                }
                if (verboseLogs) loggerApi.info("[PDD Template Export] Dataset already exists in Cloud EDL: ${rowKey}")
            }
